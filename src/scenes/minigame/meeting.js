...
// ── 픽셀 소품·모니터 베젤 → src/components/pixel-office.js 로 공통화 (메인화면과 비주얼 통일) ──
...
export function renderMeetingGame(root, state, actions, game) {
  ...
  function showResult(tier, errors, trapsLeft, usedSec) {
    ...
    btnInner.addEventListener("click", () => { playClickSfx(); cleanup(); actions.applyResult(tier, `회의 준비 ${MINI_TIER_LABEL[tier] ?? tier}: 오류 ${errors}개`, usedSec); });
