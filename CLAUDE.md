...
| 아이템 | 효과 | 사용 제한 | 부작용 |
|---|---|---|---|
| 담배 🚬 | 스트레스 -12, 게임시간 소폭 감소 | 게임당 최대 1회 | 30% 확률로 흡연횟수초과 이벤트 |
| 커피 ☕ | 체력 +15 | 게임당 최대 1회 | 연속 2회 사용 시 커피과다복용 이벤트 |
| 홍삼스틱 🧴 | 체력 +25 | 게임당 최대 1회 | — |
| 유튜브 쇼츠 📱 | 스트레스 -10 | 게임당 최대 3회 | 사용 후 다음 메인화면 상사 이벤트 발생 확률 +30% |

> **사용 제한 구현 (enforced)**: 누적 사용 횟수는 `state.counters.itemUses[itemId]`에 기록되고, `items.js`의 `maxUses`(커피 1·담배 1·쇼츠 3·홍삼 1)를 기준으로 `main.js`의 `useItem`에서 강제된다. 제한 있는 아이템은 메인화면 아이템 버튼 **우측 하단 주황 배지(남은 횟수)** 로 표시되고, 0회가 되면 비활성화된다. 단, **보상·인센티브 이벤트로 받은 아이템은 `itemUses`가 0으로 초기화**되어 다시 사용 가능하다(단일 사용 + 보충 모델).
